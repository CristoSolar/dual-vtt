...
describe('completeness', () => {
  ...
  it('has 18 subclasses, two per class', () => {
    expect(subclasses).toHaveLength(18);
    for (const characterClass of classes) {
      const pair = subclasses.filter((s) => s.classId === characterClass.id);
  ...
  it('keeps class.domains and domain.classes in agreement', () => {
    for (const domain of domains) {
      for (const classId of domain.classes) {
        ...
        expect(characterClass?.domains, `${classId} should access ${domain.id}`).toContain(
          domain.id,
        );
      }
    }
...
describe('formatters', () => {
  it('renders structured dice as SRD notation', () => {
  ...
  it('renders flat modifiers with an explicit sign', () => {
